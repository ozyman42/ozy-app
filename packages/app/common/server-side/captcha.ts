...
import { googleIsSetup } from './google';
...
export async function validateCaptcha(token: string): Promise<boolean> {
  await googleIsSetup;
  const googleCloudProjectId = await getSecret(Secrets.GoogleCloudProjectId);
  const recaptchaId = await getSecret(Secrets.RecaptchaId);
  // Create the reCAPTCHA client.
  // TODO: Cache the client generation code (recommended) or call client.close() before exiting the method.
  const client = new RecaptchaEnterpriseServiceClient();
  const projectPath = client.projectPath(googleCloudProjectId);
